import { Divider, Grid, IconButton, InputAdornment, List, ListItem, ListItemIcon, ListItemText, TextField, Typography } from '@mui/material'
...
export default function Template() {
  const [questionType, updateTypePerc] = useState([{ name: "MCQ", percent: 50 }, { name: "T/F", percent: 50 }])
  const [percType, editPercType] = useState(questionType.map((value) => false));

  const [difficulty, updateDiffPerc] = useState([{ level: "Easy", percent: 50 }, { level: "Medium", percent: 30 }, { level: "Hard", percent: 20 }])
                    ...
                    <Grid item xs={3}>
                      {percType[index] ? <TextField
                        ...
                        sx={{
                          ...
                          width: "100%",
                          marginTop:"2px"
                        }}></TextField> : <ListItemText primary={value.percent} sx={{ color: "#6B6767", width: '100%',fontSize: "15px", textTransform: 'none' }}></ListItemText>}
                    </Grid>
                  </Grid>
                </ListItem>
              </Box>
            ...
            return (
              <Box>
                ...
                secondaryAction={
                  ...
                  sx={{marginLeft: "2px",}}>
                    <Grid item xs={3}>
                      <ListItemText primary={value.level} 
                      sx={{ color: "#6B6767", width: '100%', 
                      fontSize: "15px", textTransform: 'none' 
                      }}></ListItemText>
                    </Grid>
